-- Query to find the number of active prime members at the end of 2020 in each marketplace
...
SELECT *,
       DATE_ADD(event_date, INTERVAL subscription_period MONTH) AS validity
FROM cte
WHERE rn = 1
  AND event != 'C'
  AND DATE_ADD(event_date, INTERVAL subscription_period MONTH) >= '2020-12-31';

-- Display the contents of subscription_history for reference
